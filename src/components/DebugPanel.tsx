      ...
      <div className="debug-section">
        ...
        <div className="debug-buttons">
          ...
          <button 
            className="debug-button"
            onClick={() => {
              addLog('🔧 ПРОВЕРКА ГОТОВНОСТИ К ТЕСТИРОВАНИЮ:');
              addLog('');
              addLog('✅ Checklist для тестирования:');
              addLog('');
              addLog('1. 🗄️ Сервер USA добавлен в БД');
              addLog('   → Проверьте таблицу servers в Supabase');
              addLog('');
              addLog('2. 🔑 Пароль от 3x-ui панели указан');
              addLog('   → xui_password должен быть реальным');
              addLog('');
              addLog('3. 🆔 Inbound ID указан правильно');
              addLog('   → Проверьте в панели 62.60.178.134:2053');
              addLog('');
              addLog('4. 🎫 Промокоды добавлены');
              addLog('   → TEST100_30 для бесплатной подписки');
              addLog('');
              addLog('💡 Если все готово - попробуйте промокод TEST100_30!');
            }}
            disabled={loading}
          >
            <Settings size={14} />
